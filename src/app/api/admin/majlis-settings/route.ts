...
// GET - Load Majlis webinar settings
export async function GET(request: NextRequest) {
  const settings = await loadWebinarSettings('majlis');
  return NextResponse.json(settings);
}

// PUT - Update Majlis webinar settings
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    let setting = await prisma.majlisWebinarSetting.findFirst();

    if (setting) {
      setting = await prisma.majlisWebinarSetting.update({
        where: { id: setting.id },
        data: {
      ...
      setting = await prisma.majlisWebinarSetting.create({
        data: {
          youtubeVideoId: videoId,
          webinarTitle: webinar_title || 'Majlis Webinar',
    ...
    await redis.set(KEYS.majlisSettings, JSON.stringify(cacheData));

    // Emit event for real-time updates
    wsEvents.emit('settings_updated', { type: 'majlis' });

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error('Update majlis settings error:', error);
